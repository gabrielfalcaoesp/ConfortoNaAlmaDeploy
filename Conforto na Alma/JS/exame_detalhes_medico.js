...
const url = '/Medicos/Exames/Detalhes/';
const agendamentosContainer = document.getElementById('agendamentosCliente');
    ...
    var id_medico = 1
    var detalhesAgendamentoString = localStorage.getItem("detalhesExame");
    var detalhesAgendamento = JSON.parse(detalhesAgendamentoString);
    var id_agendamento_consulta = detalhesAgendamento[0]
    ...
    const options = {
        ...
        body: JSON.stringify({ id_agendamento_consulta }), // Enviando o ID no corpo da requisição
      };
      ...
      fetch(url, options)
        ...
        .then(data => {
            ...
            console.log(data)
          document.getElementById('nomeProfissionalSalvo').innerText = data[0];
        })
        .catch(error => {
